...
static _DemoCreateWindowAsChild(void)
{
...
void STemWin_WMFun_Test()  //窗口函数测试
{
	GUI_SetBkColor(GUI_BLACK);
	WM_EnableMemdev(WM_HBKWIN);  //使用存储设备管理背景窗口
	while(1)
	{
		_DemoSetDesktopColor();	//设置窗口的背景颜色,用来演示WM_SetDesktopColor()函数
		_DemoCreateWindow();//创建窗口的演示函数,主要演示WM_CreateWindow
		_DemoCreateWindowAsChild();//创建子窗口，主要演示WM_CreateWindowAsChild()函数
		_DemoInvalidateWindow();//让指定的窗口无效，主要演示WM_InvalidateWindow()函数
		_DemoBringToTop();//将指定窗口放在其他同属窗口的前面，主要展示WMBringToTop()函数的使用
		_DemoMoveTo();//移动窗口到指定地方，主要展示WM_MoveTo()函数
		_DemoBringToBottom();//将指定窗口放在其他同属窗口的后面，主要展示WMBringToBottom()函数的使用
		_DemoMoveWindow();//移动窗口指定的距离，主要展示WM_MoveWindow()函数的使用
		_DemoHideShowParent();//隐藏和显示父窗口,主要演示WM_HideWindow()和WM_ShowWindow()函数的使用
		_DemoHideShowChild();//隐藏和显示子窗口,主要演示WM_HideWindow()和WM_ShowWindow()函数的使用
		_DemoClipping();//演示窗口裁剪功能
		_DemoRedrawing();//演示使用回调函数来实现窗口的重绘
		_DemoResizeWindow();//演示缩放窗口，主要是WM_ResizeWindow()函数的使用
		_DemoSetCallback();//设置回调函数，主要演示WM_SetCallback()函数的使用
		_DemoDeleteWindow();//删除窗口
	}
}
