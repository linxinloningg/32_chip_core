...
int dispbmp(u8 *BMPFileName,u8 mode,u32 x,u32 y,int member,int denom)
{
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			...
			{
				//在LCD中间显示图片
				GUI_BMP_Draw(bmpbuffer,(tftlcd_data.width-XSize)/2-1,(tftlcd_data.height-YSize)/2-1);
			}else //否则图片需要缩放
			{
...
int dispbmpex(u8 *BMPFileName,u8 mode,u32 x,u32 y,int member,int denom)
{
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			...
			{
				//在LCD中间显示图片
				GUI_BMP_DrawEx(BmpGetData,&BMPFile,(tftlcd_data.width-XSize)/2-1,(tftlcd_data.height-YSize)/2-1);
			}else //否则图片需要缩放
			{
				Xflag = (float)XSize*((float)member/(float)denom);
				Yflag = (float)YSize*((float)member/(float)denom);
...
void bmpdisplay_demo(void)
{
	...
	while(1)
	{
		GUI_DispStringAt("在指定位置显示一张加载到RAM中的BMP图片",0,5);
		dispbmp("0:/EMWINPICTURE/PICTURE/BMP/2.bmp",1,0,	0,	1,1);
		GUI_Delay(1000);
		GUI_Clear();
	
		GUI_DispStringAt("在LCD中间显示一张缩小1/2的加载到RAM中的BMP图片",0,5);
		dispbmp("0:/EMWINPICTURE/PICTURE/BMP/2.bmp",1,0,	0,	1,2);
		GUI_Delay(1000);
		GUI_Clear();
	
		GUI_DispStringAt("在LCD中间显示一张扩大4/3倍的加载到RAM中的BMP图片",0,5);
		dispbmp("0:/EMWINPICTURE/PICTURE/BMP/2.bmp",1,0,0,	4,3);
		GUI_Delay(1000);
		GUI_Clear();
		
		GUI_DispStringAt("在指定位置显示一张无需加载的BMP图片",0,5);
		dispbmpex("0:/EMWINPICTURE/PICTURE/BMP/2.bmp",0,0,0,	1,1);
		GUI_Delay(1000);
		GUI_Clear();
	
	
		GUI_DispStringAt("在LCD中间显示一张缩小1/2的无需加载的BMP图片",0,5);
		dispbmpex("0:/EMWINPICTURE/PICTURE/BMP/2.bmp",0,0,0,	1,2);
		GUI_Delay(1000);
		GUI_Clear();
	
		GUI_DispStringAt("在LCD中间显示一张扩大4/3倍的无需加载的BMP图片",0,5);
		dispbmpex("0:/EMWINPICTURE/PICTURE/BMP/2.bmp",0,0,0,	4,3);
		GUI_Delay(1000);
		GUI_Clear();
	}
}
