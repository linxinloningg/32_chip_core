...
u8 FT5206_Scan(u8 mode)
{
	...
	if((mode&0XF)&&((mode&0XF)<6))
	{
		...
		for(i=0;i<2;i++)
		{
			if(tp_sta&(1<<i))	//触摸有效?
			{
				...
				{
					TouchData.lcdy=320-(((u16)(buf[0]&0X0F)<<8)+buf[1]);
					TouchData.lcdx=480-(((u16)(buf[2]&0X0F)<<8)+buf[3]);
				}
				else
				{
					TouchData.lcdy=480-(((u16)(buf[2]&0X0F)<<8)+buf[3]);
					TouchData.lcdx=320-(((u16)(buf[0]&0X0F)<<8)+buf[1]);
				}  
				//if((buf[0]&0XF0)!=0X80)TouchData.lcdx=TouchData.lcdy=0;//必须是contact事件，才认为有效
				//printf("x[%d]:%d,y[%d]:%d\r\n",i,TouchData.lcdx,i,TouchData.lcdy);
			}			
		} 
		res=0;
		if(TouchData.lcdx==0 && TouchData.lcdy==0)mode=0;	//读到的数据都是0,则忽略此次数据
	...
	{ 
		tp_sta=0;
		TouchData.lcdx=0xffff;
		TouchData.lcdy=0xffff;
	} 	
	return res;
}
