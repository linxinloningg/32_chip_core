...
{
	S1216F8BD_baudrate *cfg_prt=(S1216F8BD_baudrate *)USART2_TX_BUF;
	...
	S1216F8BD_Send_Date((u8*)cfg_prt,sizeof(S1216F8BD_baudrate));//发送数据给S1216F8BD   
	delay_ms(200);				//等待发送完成 
	USART2_Init(BAUD_id[baud_id]);	//重新初始化串口2  
	return S1216F8BD_Cfg_Ack_Check();//这里不会反回0,因为UBLOX发回来的应答在串口重新初始化的时候已经被丢弃了.
} 
//配置S1216F8BD_GPS模块的时钟脉冲宽度
//width:脉冲宽度1~100000(us)
//返回值:0,发送成功;其他,发送失败.
u8 S1216F8BD_Cfg_Tp(u32 width)
{
	u32 temp=width;
	S1216F8BD_pps_width *cfg_tp=(S1216F8BD_pps_width *)USART2_TX_BUF;
	...
	S1216F8BD_Send_Date((u8*)cfg_tp,sizeof(S1216F8BD_pps_width));//发送数据给NEO-6M  
	return S1216F8BD_Cfg_Ack_Check();
}
//配置S1216F8BDF8-BD的更新速率	   
//Frep:（取值范围:1,2,4,5,8,10,20,25,40,50）测量时间间隔，单位为Hz，最大不能大于50Hz
//返回值:0,发送成功;其他,发送失败.
u8 S1216F8BD_Cfg_Rate(u8 Frep)
{
	S1216F8BD_PosRate *cfg_rate=(S1216F8BD_PosRate *)USART2_TX_BUF;
	...
	S1216F8BD_Send_Date((u8*)cfg_rate,sizeof(S1216F8BD_PosRate));//发送数据给NEO-6M 
	return S1216F8BD_Cfg_Ack_Check();
}
//发送一批数据给S1216F8BDF8-BD，这里通过串口2发送
//dbuf：数据缓存首地址
//len：要发送的字节数
