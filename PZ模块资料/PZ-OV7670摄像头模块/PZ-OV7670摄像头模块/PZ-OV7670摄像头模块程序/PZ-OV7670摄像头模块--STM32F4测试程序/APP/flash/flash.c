...
#ifdef _malloc_H	
	u8 * EN25QXX_BUF;
    EN25QXX_BUF = mymalloc(0,4096);                  //开辟一个内存块
    if(EN25QXX_BUF == 0)
    {
        return;
    }
...
#else
		EN25QXX_Read(EN25QXX_BUFFER,secpos*4096,4096);
#endif
...
#else
			if(EN25QXX_BUFFER[secoff+i]!=0XFF)break;//需要擦除
#endif			
		}
		if(i<secremain)//需要擦除
		{
...
#else
				EN25QXX_BUFFER[i+secoff]=pBuffer[i];
#endif				
			}
#ifdef _malloc_H
			EN25QXX_Write_NoCheck(EN25QXX_BUF,secpos*4096,4096);//写入整个扇区  
#else
			EN25QXX_Write_NoCheck(EN25QXX_BUFFER,secpos*4096,4096);//写入整个扇区
#endif				
