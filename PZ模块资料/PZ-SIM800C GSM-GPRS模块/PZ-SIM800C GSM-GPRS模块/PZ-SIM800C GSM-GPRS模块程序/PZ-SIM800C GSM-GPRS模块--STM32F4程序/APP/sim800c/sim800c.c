...
u8 sim800c_get_keynum(u16 x,u16 y)
{
	...
	u8 key=0;	
	if(TOUCH_Scan() == 0)			//触摸屏被按下
	{	
		for(i=0;i<5;i++)
		{
			for(j=0;j<3;j++)
			{
			 	if(TouchData.lcdx<(x+j*80+80)&&TouchData.lcdx>(x+j*80)&&TouchData.lcdy<(y+i*28+28)&&TouchData.lcdy>(y+i*28))
				{	
					key=i*3+j+1;	 
					break;	 		   
				}
		...
		{ 
			if(key<13)
			{
				...
				{ 
					...
					usart3_printf("AT+CLDTMF=2,\"%c\"\r\n",kbd_tbl[key-1][0]); 
					delay_ms(55);//延时
					usart3_printf("AT+CTTS=2,\"%c\"\r\n",kbd_tbl[key-1][0]); //TTS语音		
				}else if(cmode==2)//通话中
				{ 
					usart3_printf("AT+CLDTMF=2,\"%c\"\r\n",kbd_tbl[key-1][0]);
					delay_ms(100);
					usart3_printf("AT+VTS=%c\r\n",kbd_tbl[key-1][0]); 
...
{ 
	...
	while(1)
	{
		...
		{  
			...
			{ 
				if(key==10||key==12) usart3_printf("AT+CTTS=0\r\n");
				if(key==13)
				{usart3_printf("AT+CTTS=0\r\n");if(msglen)msglen--;}//删除  
				...
				{ 
					...
					if(sim800c_send_cmd(p,"+CMGR:",200)==0)//读取短信
					{
						...
						usart3_printf("AT+CTTS=2,\"%s\"\r\n",p);        //TTS读取短信 ASCII模式
						smsreadsta=1;								//标记有显示短信内容 
					}else
					{
						LCD_ShowFont12Char(30,75,"无短信内容!!!请检查!!");
						usart3_printf("AT+CTTS=2,\"无短信内容请检查\"\r\n");
						delay_ms(1000);
						delay_ms(1000);
				...
				if(key==15){usart3_printf("AT+CTTS=0\r\n"); break;}
			} 
			msgindex[msglen]=0; 
		...
		{   
			...
			}else
			{
				...
				{  
					...
					{ 		 				 													 
						usart3_printf("%s",p1);		 						//发送短信内容到GSM模块 
