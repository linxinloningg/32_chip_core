		...
		{	
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					fac=(float)d1/d2;
					if(fac<0.95||fac>1.05||d1==0||d2==0)//if(fac<0.10||fac>2.95||d1==0||d2==0)//不合格
					{
						cnt=0;
					...
					fac=(float)d1/d2;
					if(fac<0.95||fac>1.05)//if(fac<0.10||fac>2.95)//不合格
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(tftlcd_data.width-20,tftlcd_data.height-20,WHITE);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
...
u8 TP_Init(void)
{
	...
	tp_dev.touchtype|=tftlcd_data.dir&0X01;//横屏还是竖屏
	//CTTouch_Adjust();
	return 0;
#else
